...
class BanditEnv():
	...
	def __init__(self,means,standard_deviations=None,df=None,stations=None):
	...
	def pull_arm(self,pull,t):

		arm=pull
		mu=self.means[arm]
		sigma=self.standard_deviations[arm]
		try:
			reward=self.df[self.df['station_name'] == self.stations[arm]]
			reward=reward[reward['Day of Year'] == t]['total'].values[0]
		except:
			reward=0
		self.rewards[arm].append(reward)
		self.regret.append(self.regret[-1]+self.means[self.best_arm]-self.means[arm])
